...
class Problem:
    ...
    def create(
        ...
        for k in range(self.N + 1):
            ...
            # self.add_cost(cs.sumsqr(dL[2, k]) * 1e6 * dt)
            self.add_cost(cs.sumsqr(L[:, k]) * 1e4 * dt)
            # self.add_cost(cs.sumsqr(L[2, k]) * 1e6 * dt)
            self.add_cost(cs.sumsqr(dS[:12, k]) * 5e5 * dt)
            self.add_cost(cs.sumsqr(dS[-12:, k]) * 1e5 * dt)
            self.add_cost(cs.sumsqr(S[:, k] - initial_condition[7:]) * 1e7 * dt)
            self.add_cost(cs.sumsqr(F[:, k]) * 1e0 * dt)
            # self.add_cost(cs.sumsqr(dF[:, k]) * 1e-3 * dt)
            self.add_cost(cs.sumsqr(T[:, k]) * 2e3 * dt)
            self.add_cost(cs.sumsqr(Q[:, k] - [0, 0, 0, 1]) * 1e7 * dt)
