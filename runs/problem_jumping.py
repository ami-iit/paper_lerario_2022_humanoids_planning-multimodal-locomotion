...
def slerp(p0, p1, t):
    ...
    so = np.sin(omega)
    print(t)
    print(np.sin((1.0 - t) * omega) / so * p0 + np.sin(t * omega) / so * p1)
...
class Problem:
    ...
    def create(
        ...
        final_CoM = np.array([-0.1, 0.0, 0.57])

        opti.subject_to(opti.bounded(-1e-3, Q[:, 0] - initial_condition[3:7], 1e-3))
        opti.subject_to(opti.bounded(-1e-2, Q[:, -1] - [0, 0, 0, 1], 1e-2))
        # opti.subject_to(opti.bounded(-1e-1, Q[:, self.N / 2] - [1, 0, 0, 0], 1e-1))

        # for k in range(int(self.N / 2) + 1):
        #     opti.set_initial(Q[:, k], slerp([0, 0, 0, 1], [0, 0, 1, 0], k / self.N))

        # for k in range(int(self.N / 2), self.N + 1):
        #     opti.set_initial(
        #         Q[:, k],
        #         slerp([0, 0, 0, 1], [0, 0, 0, 1], (k) / self.N) - 0.5,
        #     )

        # final COM constraint
        opti.subject_to(R[2, self.N / 2] >= 0.70)
        ...
        for k in range(self.N + 1):
            ...
            opti.subject_to(opti.bounded(0, r[2], 1.5))
            for frame, contact in self.model.contacts_dict.items():
                ...
                opti.subject_to(
                    opti.bounded([-0.3, -0.4, -0.6], c - r, [0.3, 0.4, 0.6])
                )
        ...
        self.add_cost(cs.sumsqr(S[:, 0] - initial_condition[7:]) * 1e5)
        self.add_cost(cs.sumsqr(S[:, -1] - initial_condition[7:]) * 1e5)

        # self.add_cost(5e4 * cs.sumsqr((SO3(Q[:, 0]) - SO3(initial_condition[3:7])).vec))
        ...
        #             SO3(Q[:, -1])
        #             - (
        #                 SO3(initial_condition[3:7])
        #                 + SO3Tangent(np.array([0, np.pi, 0]))
        #             )
        #         ).vec
        #     )
        # )
        # self.add_cost(
        #     1e3 * cs.sumsqr((SO3(Q[:, -1]) - SO3(initial_condition[3:7])).vec)
        # )
        # self.add_cost(1e3 * cs.sumsqr((SO3(Q[:, 0]) - SO3(initial_condition[3:7])).vec))
        ...
        self.add_cost(cs.sumsqr(F[:, 0]) * 1e1)
        self.add_cost(cs.sumsqr(F[:, -1]) * 1e1)

        for k in range(self.N + 1):
            self.add_cost(cs.sumsqr(ddR[:, k]) * 1e0 * dt)
            self.add_cost(cs.sumsqr(dR[:2, k]) * 1e0 * dt)
            self.add_cost(cs.sumsqr(dL[:, k]) * 1e0 * dt)
            self.add_cost(cs.sumsqr(L[:, k]) * 1e0 * dt)
            self.add_cost(cs.sumsqr(dS[:-12, k]) * 1e4 * dt)
            self.add_cost(cs.sumsqr(dS[-12:, k]) * 1e4 * dt)
            self.add_cost(cs.sumsqr(S[:-12, k] - initial_condition[7:-12]) * 2e1 * dt)
            self.add_cost(cs.sumsqr(S[-12:, k] - initial_condition[-12:]) * 2e1 * dt)
            self.add_cost(cs.sumsqr(dF[:, k]) * 1e-1 * dt)
            self.add_cost(cs.sumsqr(F[:, k]) * 1e0 * dt)
    ...
    def set_planar_complementarity(self, opti, f, c, c_next):
        ...
        self.add_cost(panik * 5e4)
        opti.subject_to(opti.bounded(0, panik, 1e-3))
